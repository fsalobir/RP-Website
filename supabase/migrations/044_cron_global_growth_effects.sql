-- Cron : utiliser global_growth_effects (tableau d'effets) au lieu des 10 clés fixes pour la croissance PIB/population.
-- Si global_growth_effects est absent ou vide, les taux globaux sont 0 (rétrocompatibilité : migrer 043 avant).
...
BEGIN
  ...
  -- 3) Mise à jour des pays : taux globaux (global_growth_effects) + country_effects + bonus budget + mobilisation
  WITH
  -- Taux PIB et population issus des effets globaux (par pays : base + per_stat × stat du pays)
...
COMMENT ON FUNCTION public.run_daily_country_update() IS
  'Cron quotidien : snapshot country_history, évolution score mobilisation, mise à jour pays (global_growth_effects + country_effects + mobilisation + bonus budget), décrément country_effects.';