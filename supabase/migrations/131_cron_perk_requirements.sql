-- Cron : avantages actifs via perk_requirements (stat, gdp, population).
-- Les avantages avec un requis influence ou law_level ne sont pas appliqués dans le cron (évalués côté app).
...
BEGIN
  ...
  -- 3) Mise à jour des pays : taux globaux + country_effects + mobilisation + bonus budget + avantages (perk_effects)
  WITH
  global_growth_rates AS (
    SELECT
      ...
      SUM(
        CASE
          WHEN e->>'effect_kind' = 'gdp_growth_base' THEN (e->>'value')::numeric
          WHEN e->>'effect_kind' = 'gdp_growth_per_stat' THEN
            (e->>'value')::numeric * CASE e->>'effect_target'
              WHEN 'militarism' THEN COALESCE(c.militarism, 0)
              WHEN 'industry' THEN COALESCE(c.industry, 0)
              WHEN 'science' THEN COALESCE(c.science, 0)
              WHEN 'stability' THEN COALESCE(c.stability, 0)
              ELSE 0
            END
          ELSE 0
        END
      ...
      SUM(
        CASE
          WHEN e->>'effect_kind' = 'population_growth_base' THEN (e->>'value')::numeric
          WHEN e->>'effect_kind' = 'population_growth_per_stat' THEN
            (e->>'value')::numeric * CASE e->>'effect_target'
              WHEN 'militarism' THEN COALESCE(c.militarism, 0)
              WHEN 'industry' THEN COALESCE(c.industry, 0)
              WHEN 'science' THEN COALESCE(c.science, 0)
  ...
  -- Avantages actifs : requis dynamiques (perk_requirements). Stat/gdp/population évalués ici ; influence/law_level uniquement côté app.
  active_perk_effects AS (
    SELECT
      ...
      pe.value
    FROM public.countries c
    INNER JOIN public.perks p ON p.id NOT IN (
      SELECT pr.perk_id FROM public.perk_requirements pr WHERE pr.requirement_kind IN ('influence', 'law_level')
    )
    INNER JOIN public.perk_effects pe ON pe.perk_id = p.id
    WHERE (
      NOT EXISTS (SELECT 1 FROM public.perk_requirements pr2 WHERE pr2.perk_id = p.id)
      OR NOT EXISTS (
        SELECT 1 FROM public.perk_requirements pr2
        WHERE pr2.perk_id = p.id
        AND (
          (pr2.requirement_kind = 'stat' AND pr2.requirement_target = 'militarism' AND COALESCE(c.militarism, 0) < pr2.value)
          OR (pr2.requirement_kind = 'stat' AND pr2.requirement_target = 'industry' AND COALESCE(c.industry, 0) < pr2.value)
          OR (pr2.requirement_kind = 'stat' AND pr2.requirement_target = 'science' AND COALESCE(c.science, 0) < pr2.value)
          OR (pr2.requirement_kind = 'stat' AND pr2.requirement_target = 'stability' AND COALESCE(c.stability, 0) < pr2.value)
          OR (pr2.requirement_kind = 'gdp' AND COALESCE(c.gdp, 0) < pr2.value)
          OR (pr2.requirement_kind = 'population' AND COALESCE(c.population, 0) < pr2.value)
        )
      )
    )
  ),
  ...
  country_updates AS (
    SELECT
      ...
      COALESCE(pe.rate, 0) + COALESCE(pem.rate, 0) + COALESCE(pep.rate, 0) AS pop_effect_rate,
      COALESCE(ge.rate, 0) + COALESCE(gem.rate, 0) + COALESCE(gep.rate, 0) AS gdp_effect_rate,
      COALESCE(se.delta_mil, 0) + COALESCE(sem.delta_mil, 0) + COALESCE(seg.delta_mil, 0) + COALESCE(sep.delta_mil, 0) AS delta_mil,
      COALESCE(se.delta_ind, 0) + COALESCE(sem.delta_ind, 0) + COALESCE(seg.delta_ind, 0) + COALESCE(sep.delta_ind, 0) AS delta_ind,
      COALESCE(se.delta_sci, 0) + COALESCE(sem.delta_sci, 0) + COALESCE(seg.delta_sci, 0) + COALESCE(sep.delta_sci, 0) AS delta_sci,
      COALESCE(se.delta_stab, 0) + COALESCE(sem.delta_stab, 0) + COALESCE(seg.delta_stab, 0) + COALESCE(sep.delta_stab, 0) AS delta_stab,
...
COMMENT ON FUNCTION public.run_daily_country_update() IS
  'Cron quotidien : snapshot country_history, évolution score mobilisation, mise à jour pays (global_growth_effects + country_effects + mobilisation + bonus budget + avantages perk_effects via perk_requirements), décrément country_effects.';
