-- run_daily_country_update : utiliser la vue cron_budget_bonuses pour le calcul budget (effects/gravité).
-- Réduit la taille du corps de la fonction pour que le CLI puisse appliquer la migration.
...
COMMENT ON FUNCTION public.run_daily_country_update() IS
  'Cron quotidien : snapshot country_history, évolution score mobilisation, logs country_update_logs (inputs + pop_total_rate/gdp_total_rate + avant/après), mise à jour pays, décrément country_effects, avancement date du monde. Budget via vue cron_budget_bonuses (effects/gravité).';
