-- Events IA : distance_modes (Voisins, Continent, Monde) et verrou advisory dans run_ai_events_cron.

-- Helper : retourne les ids des pays éligibles comme cibles pour un émetteur donné, selon ai_events_config (cibles autorisées + distance_modes).
...
BEGIN
  ...
  -- distance_modes : défaut ['world'] si absent ou vide
  SELECT COALESCE(
...
COMMENT ON FUNCTION public.get_eligible_ai_event_target_ids(uuid, jsonb) IS 'Cibles éligibles pour un event IA (union world / continent / voisins selon ai_events_config.distance_modes).';

-- Réécriture de run_ai_events_cron avec distance_modes et verrou advisory
...
COMMENT ON FUNCTION public.run_ai_events_cron() IS 'Génère des events IA (ai_event_requests) selon ai_events_config (distance_modes: voisins, continent, monde). Verrou advisory pour éviter runs concurrents. Ne traite pas les conséquences différées (job Process due côté app).';
