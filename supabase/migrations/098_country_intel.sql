-- Table d'intelligence militaire : niveau d'intel par paire (observateur, cible)

CREATE TABLE IF NOT EXISTS public.country_intel (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  observer_country_id uuid NOT NULL REFERENCES public.countries(id) ON DELETE CASCADE,
  target_country_id uuid NOT NULL REFERENCES public.countries(id) ON DELETE CASCADE,
  intel_level numeric NOT NULL DEFAULT 0 CHECK (intel_level >= 0 AND intel_level <= 100),
  display_seed integer NOT NULL DEFAULT (random() * 2147483647)::int,
  updated_at timestamptz DEFAULT now(),
  UNIQUE(observer_country_id, target_country_id)
);

ALTER TABLE public.country_intel ENABLE ROW LEVEL SECURITY;

CREATE POLICY "country_intel_select_own"
...
CREATE POLICY "country_intel_admin_all"
  ON public.country_intel
  FOR ALL
  USING (EXISTS (SELECT 1 FROM public.admins a WHERE a.user_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.admins a WHERE a.user_id = auth.uid()));

-- Seed intel_config dans rule_parameters (si pas déjà présent)
INSERT INTO public.rule_parameters (key, value, description)
VALUES (
  'intel_config',
  '{"decay_flat_per_day": 2, "decay_pct_per_day": 5, "decay_mode": "flat", "espionage_intel_gain_base": 50}'::jsonb,
  'Configuration du système d''intelligence militaire : decay et gain espionnage.'
)
ON CONFLICT (key) DO NOTHING;

-- Intégrer le decay d'intel et le renouvellement du display_seed dans run_daily_country_update
...
COMMENT ON FUNCTION public.run_daily_country_update() IS
  'Cron quotidien : snapshot, mobilisation, logs, mise à jour pays, state_actions, relation_delta, effects, intel decay + seed renewal, date du monde.';
