-- Recrutement État-major : bonus additif linéaire selon le % budget Ministère de la Défense (country_budget.pct_defense).
-- Merge la clé points_per_pct_defense (défaut 0) dans etat_major_config pour les bases existantes.

UPDATE public.rule_parameters
SET value = jsonb_set(
  COALESCE(value, '{}'::jsonb),
  '{recrutement,points_per_pct_defense}',
  to_jsonb(COALESCE((value #>> '{recrutement,points_per_pct_defense}')::numeric, 0)),
  true
)
WHERE key = 'etat_major_config';
...
BEGIN
  ...
  -- Recrutement : militarisme + bonus budget Défense + effets, puis overflow vers extra_count
  FOR v_country_id, v_roster_unit_id IN
...
COMMENT ON FUNCTION public.run_etat_major_tick() IS 'Applique la progression État Major (Design, Recrutement avec budget Défense, Procuration, Stock) pour tous les pays ayant un focus défini.';
