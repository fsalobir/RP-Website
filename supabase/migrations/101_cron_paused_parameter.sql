-- Paramètre « jeu en pause » : si true, le cron quotidien ne fait rien (passage de jour manuel toujours possible).

INSERT INTO public.rule_parameters (key, value, description)
VALUES (
  'cron_paused',
  'false'::jsonb,
  'Si coché, le cron quotidien ne s''exécute plus automatiquement. Les jours peuvent toujours être passés manuellement en admin.'
)
ON CONFLICT (key) DO NOTHING;

-- Ajouter la vérification au début de run_daily_country_update (version country_laws de la migration 100)
...
DECLARE
  ...
  v_decay_pct numeric;
BEGIN
  SELECT COALESCE((value #>> '{}') = 'true', false) INTO v_cron_paused
  FROM public.rule_parameters WHERE key = 'cron_paused' LIMIT 1;
  IF v_cron_paused THEN
    RETURN;
  END IF;
...
COMMENT ON FUNCTION public.run_daily_country_update() IS
  'Cron quotidien : si cron_paused=true, ne fait rien. Sinon snapshot, lois (country_laws), logs, mise à jour pays, state_actions, relation_delta, effects, intel decay, date du monde.';
