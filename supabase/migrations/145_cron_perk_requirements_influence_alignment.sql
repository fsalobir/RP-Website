-- 145: Prérequis influence des perks — évaluation SQL alignée sur l’app (voir compute_country_influence_for_perk).
...
-- Perk influence prerequisites: same influence score as the app (influence.ts + modifiers from country_effects, laws, global_growth_effects), excluding sphere bonus.
CREATE OR REPLACE FUNCTION public.compute_country_influence_for_perk(p_country_id uuid)
RETURNS numeric
LANGUAGE sql
STABLE
...
WITH
cfg AS (
  SELECT COALESCE(value, '{}'::jsonb) AS cfg
  FROM public.rule_parameters
  WHERE key = 'influence_config'
  LIMIT 1
),
country_hp AS (
  SELECT
    c.id AS country_id,
    COALESCE(SUM((COALESCE(mru.base_count, 0) + COALESCE(cmu.extra_count, 0)) * COALESCE(mrul.hard_power, 0)), 0)::numeric AS hard_power_total
  FROM public.countries c
  LEFT JOIN public.country_military_units cmu ON cmu.country_id = c.id
  LEFT JOIN public.military_roster_units mru ON mru.id = cmu.roster_unit_id
  LEFT JOIN public.military_roster_unit_levels mrul
    ON mrul.unit_id = cmu.roster_unit_id
   AND mrul.level = GREATEST(0, FLOOR(COALESCE(cmu.current_level, 0) / 100.0))
  GROUP BY c.id
),
world_raw AS (
  SELECT
    c.id AS country_id,
    (COALESCE((cfg.cfg->>'mult_gdp')::numeric, 1e-9) * COALESCE(c.gdp, 0)) AS gdp_raw,
    (COALESCE((cfg.cfg->>'mult_population')::numeric, 1e-7) * COALESCE(c.population, 0)) AS pop_raw,
    (COALESCE((cfg.cfg->>'mult_military')::numeric, 0.01) * COALESCE(ch.hard_power_total, 0)) AS mil_raw,
    (
      (1 - LEAST(1, GREATEST(0, (COALESCE(c.stability, 0) + 3)::numeric / 6.0))) * COALESCE((cfg.cfg->>'stability_modifier_min')::numeric, 0)
      + LEAST(1, GREATEST(0, (COALESCE(c.stability, 0) + 3)::numeric / 6.0)) * COALESCE((cfg.cfg->>'stability_modifier_max')::numeric, 1)
    ) AS stability_mult,
    COALESCE((cfg.cfg->>'gravity_pct_gdp')::numeric, 50) AS gravity_gdp,
    COALESCE((cfg.cfg->>'gravity_pct_population')::numeric, 50) AS gravity_pop,
    COALESCE((cfg.cfg->>'gravity_pct_military')::numeric, 50) AS gravity_mil
  FROM public.countries c
  LEFT JOIN country_hp ch ON ch.country_id = c.id
  CROSS JOIN cfg
),
world_avg AS (
  SELECT
    COALESCE(AVG(gdp_raw), 0)::numeric AS avg_gdp,
    COALESCE(AVG(pop_raw), 0)::numeric AS avg_pop,
    COALESCE(AVG(mil_raw), 0)::numeric AS avg_mil
  FROM world_raw
),
base_influence AS (
  SELECT
    wr.country_id,
    (wr.gdp_raw * CASE
      WHEN wa.avg_gdp = 0 THEN 1
      ELSE LEAST(2, GREATEST(0.1, 1 + (wr.gravity_gdp / 100.0) * ((wa.avg_gdp - wr.gdp_raw) / wa.avg_gdp)))
    END) AS gdp_after_gravity,
    (wr.pop_raw * CASE
      WHEN wa.avg_pop = 0 THEN 1
      ELSE LEAST(2, GREATEST(0.1, 1 + (wr.gravity_pop / 100.0) * ((wa.avg_pop - wr.pop_raw) / wa.avg_pop)))
    END) AS pop_after_gravity,
    (wr.mil_raw * CASE
      WHEN wa.avg_mil = 0 THEN 1
      ELSE LEAST(2, GREATEST(0.1, 1 + (wr.gravity_mil / 100.0) * ((wa.avg_mil - wr.mil_raw) / wa.avg_mil)))
    END) AS mil_after_gravity,
    wr.stability_mult
  FROM world_raw wr
  CROSS JOIN world_avg wa
),
influence_modifier_effects AS (
  SELECT
    ce.country_id,
    ce.effect_kind,
    ce.value::numeric AS value
  FROM public.country_effects ce
  WHERE ce.effect_kind IN ('influence_modifier_global', 'influence_modifier_gdp', 'influence_modifier_population', 'influence_modifier_hard_power')
    AND (ce.duration_kind = 'permanent' OR COALESCE(ce.duration_remaining, 0) > 0)
  ...
  JOIN public.rule_parameters rp_levels
    ON rp_levels.key = CASE cl.law_key
      ...
      ELSE NULL
    END
  JOIN public.rule_parameters rp_cfg
    ON rp_cfg.key = CASE cl.law_key
    ...
    END
  CROSS JOIN LATERAL (
    SELECT j.key AS level_key
    FROM jsonb_each_text(rp_cfg.value->'level_thresholds') AS j(key, val)
    WHERE (val::numeric) <= COALESCE(cl.score, 0)
    ORDER BY (val::numeric) DESC
    LIMIT 1
  ) lvl
  CROSS JOIN LATERAL jsonb_array_elements(
    COALESCE(
      CASE WHEN jsonb_typeof(rp_levels.value) = 'array' THEN rp_levels.value ELSE rp_levels.value->'value' END,
      '[]'::jsonb
    )
  ) AS e
  WHERE (e->>'level') = lvl.level_key
    AND (e->>'effect_kind') IN ('influence_modifier_global', 'influence_modifier_gdp', 'influence_modifier_population', 'influence_modifier_hard_power')
  ...
  SELECT
    ...
    public.parse_effect_value(e) AS value
  FROM public.countries c
  JOIN public.rule_parameters rp ON rp.key = 'global_growth_effects'
  CROSS JOIN LATERAL jsonb_array_elements(
    COALESCE(
      CASE WHEN jsonb_typeof(rp.value) = 'array' THEN rp.value ELSE rp.value->'value' END,
      '[]'::jsonb
    )
  ) AS e
  WHERE (e->>'effect_kind') IN ('influence_modifier_global', 'influence_modifier_gdp', 'influence_modifier_population', 'influence_modifier_hard_power')
),
mods AS (
  SELECT
    country_id,
    COALESCE(EXP(SUM(LN(value)) FILTER (WHERE effect_kind = 'influence_modifier_global' AND value > 0)), 1) AS mod_global,
    COALESCE(EXP(SUM(LN(value)) FILTER (WHERE effect_kind = 'influence_modifier_gdp' AND value > 0)), 1) AS mod_gdp,
    COALESCE(EXP(SUM(LN(value)) FILTER (WHERE effect_kind = 'influence_modifier_population' AND value > 0)), 1) AS mod_population,
    COALESCE(EXP(SUM(LN(value)) FILTER (WHERE effect_kind = 'influence_modifier_hard_power' AND value > 0)), 1) AS mod_hard_power
  FROM influence_modifier_effects
  GROUP BY country_id
)
SELECT
  COALESCE(
    round(
      (
        bi.gdp_after_gravity * COALESCE(m.mod_gdp, 1)
        + bi.pop_after_gravity * COALESCE(m.mod_population, 1)
        + bi.mil_after_gravity * COALESCE(m.mod_hard_power, 1)
      ) * bi.stability_mult * COALESCE(m.mod_global, 1),
      12
    ),
    0
  )
FROM base_influence bi
LEFT JOIN mods m ON m.country_id = bi.country_id
WHERE bi.country_id = p_country_id;
$$;

COMMENT ON FUNCTION public.compute_country_influence_for_perk(uuid) IS
  'Score d''influence pour les prérequis de perks (parité app : influence.ts + modificateurs pays/lois/global), hors bonus de sphère.';
...
COMMENT ON FUNCTION public.run_daily_country_update() IS
  'Cron quotidien : snapshot, lois, logs, mise à jour pays (global + country_effects + lois + IA + idéologie + avantages, avec perks à prérequis influence alignés), État Major, state_actions, relation_delta, effects, intel decay, date du monde.';
