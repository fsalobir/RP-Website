-- Supprimer le scale (×50, ×100) et le plafond stabilité : la magnitude du changement
-- vient des règles globales (bonus par ministère). Formule = avant + delta_effets + budget (sans multiplicateur).
...
BEGIN
  ...
  SELECT COALESCE((value #>> '{}')::numeric, 0.001) INTO pop_base FROM public.rule_parameters WHERE key = 'population_growth_base_rate' LIMIT 1;
  SELECT COALESCE((value #>> '{}')::numeric, 0.0005) INTO gdp_base FROM public.rule_parameters WHERE key = 'gdp_growth_base_rate' LIMIT 1;
  ...
  budget_bonuses AS (SELECT b.country_id, (b.pct_sante/100.0)*COALESCE((SELECT (value->'bonuses'->>'population')::numeric FROM public.rule_parameters WHERE key = 'budget_sante' LIMIT 1), 0) AS pop_rate, (b.pct_infrastructure/100.0)*COALESCE((SELECT (value->'bonuses'->>'gdp')::numeric FROM public.rule_parameters WHERE key = 'budget_infrastructure' LIMIT 1), 0)+(b.pct_affaires_etrangeres/100.0)*COALESCE((SELECT (value->'bonuses'->>'gdp')::numeric FROM public.rule_parameters WHERE key = 'budget_affaires_etrangeres' LIMIT 1), 0) AS gdp_rate, (b.pct_defense/100.0)*COALESCE((SELECT (value->'bonuses'->>'militarism')::numeric FROM public.rule_parameters WHERE key = 'budget_defense' LIMIT 1), 0) AS mil_delta, (b.pct_infrastructure/100.0)*COALESCE((SELECT (value->'bonuses'->>'industry')::numeric FROM public.rule_parameters WHERE key = 'budget_infrastructure' LIMIT 1), 0)+(b.pct_industrie/100.0)*COALESCE((SELECT (value->'bonuses'->>'industry')::numeric FROM public.rule_parameters WHERE key = 'budget_industrie' LIMIT 1), 0) AS ind_delta, (b.pct_education/100.0)*COALESCE((SELECT (value->'bonuses'->>'science')::numeric FROM public.rule_parameters WHERE key = 'budget_education' LIMIT 1), 0)+(b.pct_recherche/100.0)*COALESCE((SELECT (value->'bonuses'->>'science')::numeric FROM public.rule_parameters WHERE key = 'budget_recherche' LIMIT 1), 0) AS sci_delta, (b.pct_education/100.0)*COALESCE((SELECT (value->'bonuses'->>'stability')::numeric FROM public.rule_parameters WHERE key = 'budget_education' LIMIT 1), 0)+(b.pct_interieur/100.0)*COALESCE((SELECT (value->'bonuses'->>'stability')::numeric FROM public.rule_parameters WHERE key = 'budget_interieur' LIMIT 1), 0)+(b.pct_affaires_etrangeres/100.0)*COALESCE((SELECT (value->'bonuses'->>'stability')::numeric FROM public.rule_parameters WHERE key = 'budget_affaires_etrangeres' LIMIT 1), 0) AS stab_delta FROM public.country_budget b),
  ...
  SET
    ...
    militarism = LEAST(10, GREATEST(0, ROUND((COALESCE(c.militarism, 0) + u.delta_mil + u.budget_mil)::numeric, 0)))::smallint,
    industry   = LEAST(10, GREATEST(0, ROUND((COALESCE(c.industry, 0)   + u.delta_ind + u.budget_ind)::numeric, 0)))::smallint,
    science    = LEAST(10, GREATEST(0, ROUND((COALESCE(c.science, 0)    + u.delta_sci + u.budget_sci)::numeric, 0)))::smallint,
    stability  = LEAST(3, GREATEST(-3, ROUND((COALESCE(c.stability, 0) + u.delta_stab + u.budget_stab)::numeric, 0)))::smallint,
    updated_at = now()
...
COMMENT ON FUNCTION public.run_daily_country_update() IS
  'Cron quotidien : snapshot, logs, mise à jour pays. Stats et stabilité = avant + delta_effets + budget (règles = magnitude, pas de scale).';