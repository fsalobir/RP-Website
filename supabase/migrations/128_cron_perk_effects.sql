-- Cron : inclure les effets des avantages actifs (perk_effects) dans les taux de croissance et stat_delta.
-- Un avantage est actif pour un pays si pour chaque min_* non null, la stat du pays >= min_*.
...
BEGIN
  ...
  -- Avantages actifs : pays × perks × perk_effects où les stats du pays satisfont les min_*
  active_perk_effects AS (
    SELECT
      ...
      pe.value
    FROM public.countries c
    INNER JOIN public.perks p ON (
      (p.min_militarism IS NULL OR c.militarism >= p.min_militarism)
      AND (p.min_industry IS NULL OR c.industry >= p.min_industry)
      AND (p.min_science IS NULL OR c.science >= p.min_science)
      AND (p.min_stability IS NULL OR c.stability >= p.min_stability)
    )
...
COMMENT ON FUNCTION public.run_daily_country_update() IS
  'Cron quotidien : snapshot country_history, évolution score mobilisation, mise à jour pays (global_growth_effects + country_effects + mobilisation + bonus budget + avantages perk_effects), décrément country_effects.';
