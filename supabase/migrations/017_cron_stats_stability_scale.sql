-- Ajuster le cron : stabilité = bonus budget plafonné à ±1/jour pour éviter le saut à 3 ;
-- militarisme / industrie / science = scale 100 pour que des allocations typiques fassent bouger les stats.
...
BEGIN
  ...
  country_updates AS (
    SELECT c.id AS country_id,
      COALESCE(pe.rate, 0) AS pop_effect_rate, COALESCE(ge.rate, 0) AS gdp_effect_rate,
      COALESCE(se.delta_mil, 0) AS delta_mil, COALESCE(se.delta_ind, 0) AS delta_ind, COALESCE(se.delta_sci, 0) AS delta_sci, COALESCE(se.delta_stab, 0) AS delta_stab,
      COALESCE(bb.pop_rate, 0) AS budget_pop_rate, COALESCE(bb.gdp_rate, 0) AS budget_gdp_rate,
      COALESCE(bb.mil_delta, 0) AS budget_mil, COALESCE(bb.ind_delta, 0) AS budget_ind, COALESCE(bb.sci_delta, 0) AS budget_sci, COALESCE(bb.stab_delta, 0) AS budget_stab
    FROM public.countries c
  ...
  SELECT c.id, v_run_at,
    jsonb_build_object(
      'pop_base', pop_base, 'gdp_base', gdp_base,
  ...
  SET
    ...
    militarism = LEAST(10, GREATEST(0, ROUND((COALESCE(c.militarism, 0) + u.delta_mil + u.budget_mil * 100.0)::numeric, 0)))::smallint,
    industry   = LEAST(10, GREATEST(0, ROUND((COALESCE(c.industry, 0)   + u.delta_ind + u.budget_ind * 100.0)::numeric, 0)))::smallint,
    science    = LEAST(10, GREATEST(0, ROUND((COALESCE(c.science, 0)    + u.delta_sci + u.budget_sci * 100.0)::numeric, 0)))::smallint,
    stability  = LEAST(3, GREATEST(-3, ROUND((COALESCE(c.stability, 0) + u.delta_stab + LEAST(1, GREATEST(-1, u.budget_stab * 50.0)))::numeric, 0)))::smallint,
    updated_at = now()
  FROM country_updates u
  WHERE c.id = u.country_id;

  -- 5) Remplir *_after dans les logs
  UPDATE public.country_update_logs l
  SET population_after = c.population, gdp_after = c.gdp,
  ...
  -- 6) Décrément / suppression des effets
...
COMMENT ON FUNCTION public.run_daily_country_update() IS
  'Cron quotidien : snapshot, logs, mise à jour pays. Stabilité: bonus budget plafonné ±1/jour. Mil/Ind/Sci: scale 100.';