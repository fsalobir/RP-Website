-- Effets relation_delta : évolution des relations bilatérales à chaque tick du cron.
...
BEGIN
  FOR r IN
    ...
    WHERE ce.effect_kind = 'relation_delta'
      ...
      AND ce.country_id <> ce.effect_target
  LOOP
...
-- Intégrer l'appel dans le cron (réécriture de run_daily_country_update avec la nouvelle étape).
-- On insère PERFORM public.apply_relation_delta_effects(); après add_state_actions_from_effects.
...
COMMENT ON FUNCTION public.run_daily_country_update() IS
  'Cron quotidien : snapshot country_history, évolution score mobilisation, logs country_update_logs (inputs + pop_total_rate/gdp_total_rate + avant/après), mise à jour pays, crédit actions d''État (state_actions_grant), application relation_delta, décrément country_effects (sauf permanent), avancement date du monde. Budget via vue cron_budget_bonuses (effects/gravité).';
