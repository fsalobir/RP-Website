-- Events IA : en auto-accept, ne plus insérer de dice_results factices.
-- Le job Process due (route ou action admin) fera les vrais jets succès + impact côté app.
  ...
  FOR v_i IN 1 .. v_count_major LOOP
    ...
    IF COALESCE((v_auto_accept->>v_action_key)::boolean, false) THEN
      ...
      INSERT INTO public.ai_event_requests (
        country_id, action_type_id, status, payload, scheduled_trigger_at, source
      ) VALUES (
        v_emitter_id, v_action_type_id, 'accepted', v_payload, v_scheduled, 'cron'
      );
    ELSE
      INSERT INTO public.ai_event_requests (
...
COMMENT ON FUNCTION public.run_ai_events_cron(boolean) IS 'Génère des events IA selon ai_events_config. Auto-accept : status accepted sans dice_results ; le job Process due fait les jets succès/impact côté app.';
