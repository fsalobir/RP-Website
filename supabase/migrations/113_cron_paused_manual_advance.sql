-- Jeu en pause : la pause ne doit bloquer que l'appel externe (route API / cron).
-- L'appel direct à run_daily_country_update (bouton « Passer jour » en admin) doit toujours exécuter.
-- On retire donc la vérification cron_paused de la RPC ; seule la route GET /api/cron/daily-country-update la lit.
...
COMMENT ON FUNCTION public.run_daily_country_update() IS
  'Cron quotidien : snapshot, lois (country_laws), logs, mise à jour pays, state_actions, relation_delta, effects, intel decay, date du monde. La pause (cron_paused) est gérée uniquement par la route API /api/cron/daily-country-update, pas par cette RPC.';
