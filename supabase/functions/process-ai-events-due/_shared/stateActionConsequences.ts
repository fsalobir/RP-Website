...
import { getRelation, normalizePair, RELATION_MIN, RELATION_MAX } from "./relations.ts";
import { dispatchToDiscord } from "./discord-dispatch.ts";
...
async function applyImmediateEffect(
  ...
  if (kind === "military_unit_extra" && target) {
    ...
    if (existing) {
      ...
      const { error: upErr } = await supabase.from("country_military_units").update({ extra_count: newExtra }).eq("id", existing.id);
  ...
  if (kind === "military_unit_tech_rate" && target) {
    const { data: roster } = await supabase.from("military_roster_units").select("level_count").eq("id", target).single();
    ...
    if (cmu) {
      ...
      const { error: upErr } = await supabase.from("country_military_units").update({ current_level: newLevel }).eq("id", cmu.id);
      if (upErr) return { error: upErr.message };
    } else {
  ...
  if (kind === "stat_delta" && target && STAT_COLUMNS.includes(target as (typeof STAT_COLUMNS)[number])) {
    ...
    const { error: upErr } = await supabase.from("countries").update({ [col]: newVal }).eq("id", countryId);
    if (upErr) return { error: upErr.message };
    return {};
  }
  ...
  if (kind === "relation_delta" && target) {
    ...
    const { error: relErr } = await supabase
      ...
      .upsert({ country_a_id: a, country_b_id: b, value: newValue, updated_at: new Date().toISOString() }, { onConflict: "country_a_id,country_b_id" });
...
export async function applyStateActionConsequences({
  ...
  const [countryRes, targetCountryRes, worldDateRes, rosterRes, countriesRes] = await Promise.all([
    ...
    targetCountryId ? supabase.from("countries").select("name").eq("id", targetCountryId).single() : Promise.resolve({ data: null }),
  ...
  const adminEffectsSummary =
    effects.length > 0 ? effects.map((e) => formatAdminEffectLabel(e, { rosterUnits, countries: countriesList })).join("\n") : undefined;
...
export { ACTION_KEYS_REQUIRING_IMPACT_ROLL };
